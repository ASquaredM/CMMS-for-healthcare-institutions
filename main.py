...
class ApplicationWindow(hospital_gui.Ui_MainWindow):
    def __init__(self, mainWindow):
        ...
        self.question2 = [
            self.q12, self.q12, self.q13, self.q14, self.q15, self.q16, self.q17,
    ...
    def UpdateDevCombo(self):
        ...
        dailyDevices_names = [str(device[0])+" ,ID:"+str(device[1]) for device in dailyDevices_names]
    ...
    def UpdateForm(self):
        family = DB.SelectRows(
            "device",
            "DevName='{}'".format(str(self.Inspection_comboBox.currentText()).split(" ,ID:")[0]))
    ...
    def update_form2(self):
        family = DB.SelectRows(
            "device",
            "DevName='{}'".format(str(self.ppm_comboBox.currentText()).split(" ,ID:")[0]))
        if len(family) > 0:
            form = DB.SelectRows(
                "form",
            ...
            if len(form) > 0:
                ...
                for i, quest in enumerate(form):
                    if quest != "  ?":
                        ...
                        EnableVar -= 1
                self.clear_form(1,len(self.checks2), 10 - EnableVar)  ############generalize these functions ---> enable_form(self,checklist, amount) , clear_Form(self, questionList,checklist,lenlist, amount)
